...
- Twelve named tools are registered; no arbitrary URL/path/request tool exists.
...
Telephone numbers are masked by default in all call-level tools. Aggregated tools should be used for analyses whenever record-level data is unnecessary. The server deliberately excludes recordings, call notes, AI transcripts, PIN codes, and endpoint IP addresses.
